...
\begin{tikzpicture}[scale=1.0, every node/.style={transform shape}, font=\large]
  ...
  \node [block, very thick, node distance=2.5cm, text width=10.0cm, fill=lblue!30, below = 1.0 cm of pde] (discretization) {Use Spatial, Temporal Discretization Methods \\ (Deterministic Framework)};
  \node [block, very thick, node distance=2.5cm, text width=10.0cm, fill=lblue!30, below = 1.0 cm of discretization] (algebraic) {Use Probabilistic Sampling to Perform Galerkin Projection};
  ...
  \package{Semi-Intrusive Stochastic Galerkin}{(pde)(discretization)(algebraic)(evaluation)};
\end{tikzpicture}
