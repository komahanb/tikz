...
\tikzstyle{cd generalization}=[cd relation, -i>, color=black, ultra thick]
...
\tikzstyle{cd package}       =[ultra thick, rounded corners, draw=black, text=black]
...
\begin{tikzpicture}[scale=1.0, eultra node/.style={transform shape}]

  \node [block, ultra thick, node distance=2.5cm, text width=6.5cm, fill=white] (pde) at (0,0) {Stochastic Differential Equations};
  \node [block, ultra thick, node distance=2.5cm, text width=6.5cm, fill=white, below = 1.0 cm of pde] (discretization) {Use Spatial, Temporal \& \\{Probabilistic} Discretization Principles};
  \node [block, ultra thick, node distance=2.5cm, text width=6.5cm, fill=white, below = 1.0 cm of discretization] (algebraic) {Stochastic Algebraic Equations};
  \node [block, ultra thick, node distance=2.5cm, text width=6.5cm, fill=white, below = 1.0 cm of algebraic] (evaluation) {Evaluate Residuals, Jacobian, Initial \& Boundary Conditions};

  \generalization{pde}{discretization}
  \generalization{discretization}{algebraic}
  \generalization{algebraic}{evaluation}

  \package{Classical Stochastic Galerkin Method}{(pde)(discretization)(algebraic)(evaluation)}
